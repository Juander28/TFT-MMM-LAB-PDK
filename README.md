...
| 2 | `sd`     | 6/0 | Au source/drain, directly on the IGZO - the contact **is** the overlap, there is no via layer |
| 3 | `oxetch` | 5/0 | openings in the blanket 50 nm Al2O3 gate dielectric, over the S/D probe pads only |
...
library **`igzo_mmm_lab_pr`** in the library browser, with two PCells:
...
- **`cap_mim`** - the S/D-metal / Al2O3 / gate-metal overlap capacitor, the
  structure Cox came from. Its read-only `c` parameter reports 1.564 fF/µm².
...
`symbols/tft_igzo.sym`, pins **D G S B** in that order. `W`, `L` and `ov` are
drawn dimensions in metres. A testbench that sweeps the measured device is in
`libs.tech/xschem/tests/tft_iv.sch`.
...
docs/                      PDK_GUIDE.md, the extraction scripts, the original .lyp
